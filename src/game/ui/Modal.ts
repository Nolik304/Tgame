...
export function makeButton(
  ...
  const txt = scene
    ...
    .setOrigin(0.5);
  if (iconSpr) {
    // текст центрируется между иконкой и правым краем
...
/** Круглая иконка-кнопка (шестерёнка, магазин и т.п.). */
export function makeIconButton(
  ...
  const c = scene.add.container(0, 0);
  const g = scene.add.graphics();
  g.fillGradientStyle(0x2c5a40, 0x2c5a40, 0x142e20, 0x142e20, 1);
